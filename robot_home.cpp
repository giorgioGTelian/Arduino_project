...
void setup() {
...
#ifdef USE_ULTRASONIC /****use the ultrasonic sensor*******/
  ...
  pinMode(Echo, INPUT);
  procedure(90);
  procedure(0); //set the angle of the servo to 0°
#endif
...
void loop() {
  ...
  if (irrecv.decode(&results)) { //Receive infrared remote control value
    ...
    // New code for handling LED control
...
void Avoid()
{
  ...
  int maxForwardCounter = 0;
  unsigned long scanTimer = millis(); // Initialize scan timer
  ...
  while (flag == 0)
  {
    ...
    if (a < 20) {//When the distance in front is less than 20cm
      ...
      // Perform a 360-degree scan by turning the robot and using the servo
      for (int i = 0; i < 2; i++) {
        procedure(i == 0 ? 0 : 180); // Move the servo to the left or right extreme
        delay(500); // Wait for the servo to move
        
        for (int j = 0; j < 2; j++) {
          Car_T_right(); // Turn the robot to the right
          delay(450); // Adjust the delay for approx. 90-degree turn
          Car_Stop(); // Stop the robot
          delay(500); // Wait for the robot to stop
        }
      }
      ...
      if (a1 < 100 && a2 < 100 && a < 100) { //When everything is too close (< 1 m)
        Car_Stop(); //stop
        // Scan around with the servo without moving the robot
      }
      else {
      ...
      Car_front(); //go front
      forwardCounter++;
     if (forwardCounter >= maxForwardCounter) { // If the counter reaches the maximum limit
        Car_Stop(); // Stop the robot
        // Add code here to scan the surroundings, e.g., rotate the servo and check distances
        forwardCounter = 0; // Reset the counter after scanning
      }
    }
